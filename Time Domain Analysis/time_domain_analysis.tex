...
\color{blue}
\section{Second Order Circuits - RLC}
\color{black}
...
\color{blue}
\subsection{The Source-Free Parallel RLC}
\color{black}
...
\end{tcolorbox}
\color{blue}
\textbf{Graphical Form of the Overdamped Response}
\color{black}
\\~\\
 ...
 \begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \addplot[
        ...
        samples = 100,
        domain = 0:4
    ]{1.414*210*exp(-2*x)*sin(deg(1.4142*x))};
...
\begin{align*}
\frac{di_L}{dt} = e^{-\alpha t}(-B_1\omega_d\sin{\omega_d t} + B_2\omega_d\cos{\omega_d t}) - \alpha e^{-\alpha t}(B_1\cos{\omega_d t} + B_2\sin{\omega_d t})\\
\Rightarrow v_L(0) = L\frac{di_L}{dt}\Biggr|_{t=0s} = 97.30\si{\volt}
\end{align*}

This gives $B_2 = 2.561 \si{\ampere}$ and hence the response is

\begin{equation*}
i_L(t) = e^{-1.2 t}(2.027\cos{4.750 t} + 2.561\sin{4.750 t})
\end{equation*}

This response is plotted in Fig.\ref{fig:underdamp_plot_example_2}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \addplot[
        ...
        samples = 100,
        domain = 0:4
    ]{exp(-1.2*x)*(2.561*sin(deg(4.750*x)) + 2.027*cos(deg(4.750)))};
	...
	\end{axis}
	\end{tikzpicture}
  \caption{The response for the example}
  \label{fig:underdamp_plot_example_2}
\end{figure}%

\end{tcolorbox}

\color{blue}
\subsection{The Source-Free Series RLC}
\color{black}
...
\begin{tcolorbox}[colback=NavyBlue!5, colframe=NavyBlue!75!black, title=\textbf{Example 3.4}, breakable]
	
	Consider the circuit shown in Fig. \ref{fig:forced_parallel_rlc_example}. Determine the type of response and the response for the inductor current and the capacitor voltage.
	
	\begin{figure}[H]
		\centering
		\begin{circuitikz}[american]
			\draw (0,0) to [american voltage source, v=$4u(t)\si{\volt}$, fill=red] ++(0,4)  to [R=0.5<\ohm>] ++(3,0) node (a){} to [cute inductor, l=$1/3\si{\henry}$, i=$i(t)$] 
					++(0,-4) -- (0,0);
			\draw (3,4) -- ++(3,0) to [C=0.25<\farad>, v=$v(t)$, voltage shift=2.5] ++(0, -4) -- (0,0);
		\end{circuitikz}
		\caption{}
		\label{fig:forced_parallel_rlc_example}
	\end{figure}
	
	For this forced parallel RLC,
	
	\begin{align*}
		\alpha &= \frac{1}{2RC} = 4\\
		\omega_0 &= \frac{1}{\sqrt{LC}} = \sqrt{12}
	\end{align*}

	As $\alpha > \omega_0$, the response is overdamped. The initial value of the capacitor voltage and the inductor current are both zero as the inductor acts as a short
	to DC current in steady state.
	...
	Here, the value of inductor current in steady state, $i_f = 4/0.5 = 8\si{\ampere}$, and hence
	
	\begin{align*}
		i(t)&= Ae^{-6t} + Be^{-2t} + 8
	\end{align*}
	
	The initial conditions $i(0) = 0$ and $L\frac{di}{dt}\Biggr|_{t=0} = 0$ give
	
	\begin{align*}
		8 + A + B  &= 0\\
		-6A - 2B = 0\\
		\Rightarrow A = 4, B = -12 \\
		\Rightarrow i(t)&= 4e^{-6t} - 12e^{-2t} + 8\\
		v(t) = L\frac{di}{dt} &= 8e^{-2t} - 8e^{-6t}
	\end{align*}
